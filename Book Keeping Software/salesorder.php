...
if(!isset($_SESSION["BusinessName"])){
	...
	$custnamequery =mysqli_query($conn, "SELECT * FROM customers WHERE BusinessId ='$id'");
...
if(isset($_POST["submit"])){
    ...
    if(!empty($VendName) || !empty($VendEmail) ){
        if(mysqli_connect_error()){
            ...
            $INSERT = "INSERT INTO customers(BusinessId  ,CustName,CustEmail)
             Values(?,?,?)";
            ...
            if($rnum==0 ){
                ...
                $stmnt->execute();
                echo"<script> alert('New Custmer Added Successfuly!!');</script>";
                ?>
        <script type="text/javascript">
        window.location.href="salesorder.php"</script>
        <?php
            }else{
                echo"<script> alert('Customer Name Already Exisit!!');</script>";
                ?>
        <script type="text/javascript">
        window.location.href="salesorder.php"</script>
        <?php
  ...
  $sql= "INSERT INTO salesorder VALUE(null,'$id','$orderdate','$VendName','$ProductName','$ProdQuantity',
  '$Unitofmeasure','$UnitPrice','$TotalAmount','0')";
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="salesorder.php" id="home">Sales Order</a>
          </li>
...
<div class="container">
	...
	<div class="col-md-2 mx-0" id="register">
        ...
        <h2 id="formheading">Add Customer</h2>
        <center><hr id="headinghr"></center>
        </div>
        <form action="salesorder.php" method="POST">
        <input type="hidden" id="BusnameId" name="BusnameId" value="<?php echo $id?>">
            <label for="CustName" class="form-label" id="formlabel" 
               >Customer Name</label>
            <input type="text" class="form-control" id="VendName" name="VendName" 
         required>
            <label for="CustEmail" class="form-label" id="formlabel" 
      ...
      <div class="text-center">
      <h2 id="formheading">Make Sales</h2>
      <center><hr id="headinghr"></center>
        ...
        <form action="salesorder.php" method = "POST">
          ...
          <label for="VendName" class="form-label" id="formlabel">Customer Name</label>
          <select name="VendName" id="VendName" class="form-control">
<option >  </option>
<?php

if(mysqli_num_rows($custnamequery) > 0){
$name=0;
while($namerows = mysqli_fetch_assoc($custnamequery)){
$custname=$namerows['CustName'];
?>
<option value="<?php echo $custname; ?>">
<?php echo $custname; ?></option>
<?php
$name++;		
}
}
?>
	  ...
	  <div class="col-md-7 mx-0" id="register">
        ...
        <h2 id="formheading">All Sales Orders </h2>
        <center><hr id="headinghr"></center>
        </div>
	...
	<?php
$id=$_SESSION["BusinessId"];
$salesorder ="SELECT * FROM salesorder WHERE BusinessId='$id'";
$result = $conn->query($salesorder);
if(!$result){
	die("Connection Error".$conn->error);
      ...
      while($row = $result->fetch_assoc()){
		 ...
		 <tr>
          ...
          <td >
		    <a class='btn btn-primary btn-sm text-light' href='Receivepayment.php?SorderId =$row[SorderId]'>Get Paid</a>
          </td>
		  </tr>
		  ";
	  }
...
<script src="js/bootstrap.bundle.js"></script>
  ...
  <script>
		...
		function updateResult() {
            ...
            const unit = parseFloat(SellingPrice.value);
        ...
        SellingPrice.addEventListener("input", updateResult());</script>
</body>
</html>